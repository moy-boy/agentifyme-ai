...
def yaml_path(request):
    ...
    file_path = os.path.join(
        os.path.dirname(__file__), "..", "data", "prompt", filename
    )
    return file_path
...
def test_basic_prompt(yaml_path):
    ...
    prompt = prompt_template.render_as_prompt(
        persona="personal assistant", question="How are you?"
    )
    ...
    assert (
        prompt.description
        == "A basic prompt designed to ask a single question and provide a specific answer."
    )
...
def test_topic_specific_prompt(yaml_path):
    ...
    assert (
        prompt.description
        == "A prompt template designed to generate questions based on a specific topic."
    )
    ...
    expected_content = (
        "What is the current weather condition and temperature in New York?"
    )
    ...
    prompt = prompt_template.render_as_prompt(
        persona="news anchor", role="providing news", tone="serious", topic="news"
    )
...
def test_multi_topic_prompt(yaml_path):
    ...
    assert (
        prompt.description
        == "A prompt template designed to generate questions based on multiple topics of interest."
    )
    assert prompt.version == "1.0"
    assert prompt.tags is not None
    ...
    expected_system_message = (
        "As a advisor, your task is to providing guidance.\n"
    ...
    expected_system_message = (
        "As a coach, your task is to mentoring.\n"
